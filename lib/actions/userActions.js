...
export async function getUserProfile(userId) {
  try {
    ...
    // Fetch both tables sequentially
    const userResult = await supabase.from("users").select("*").eq("id", userId).single();
    const instructorResult = await supabase.from("sellers").select("*").eq("user_id", userId).single();

    console.log("Fetched user:", userResult, "Instructor:", instructorResult);

    const user = userResult.data || null;
    const instructor = instructorResult.data || null;

    // Handle real errors only (ignore 'No rows found')
    const hasUserError = userResult.error && userResult.error.code !== "PGRST116";
    const hasInstructorError = instructorResult.error && instructorResult.error.code !== "PGRST116";

    if (hasUserError) {
      console.error("User table error:", userResult.error);
      throw new Error(`Failed to fetch user profile: ${userResult.error.message}`);
    }

    if (hasInstructorError) {
      console.error("Instructor table error:", instructorResult.error);
      throw new Error(`Failed to fetch instructor profile: ${instructorResult.error.message}`);
    }

    // Update store with complete profile data
    const { setUser, setRole } = useUserStore.getState();

    // Determine role based on existence
    let role = null;
    if (user && instructor) {
      role = "both"; // User exists in both tables
      setUser(user);
      setRole(role);
    ...
    console.log("Store updated with:", { user, instructor, role });

    return {
      success: true,
      data: { user, instructor, role },
    };
  } catch (error) {
    console.error("Get user profile error:", error);

    // Clear store on error to prevent stale data
...
export async function getInstructorProfile(userId) {
  try {
    ...
    // Update store when getting instructor profile
    const { setInstructor } = useUserStore.getState();
    setInstructor(data);
