...
/**
 * Google Sign In/Sign Up - Updated with proper redirect handling
 */
...
/**
 * Handle OAuth callback - Updated to handle both URL fragments and query params
 */
export const handleOAuthCallback = async () => {
  try {
    ...
    if (data.session) {
      ...
      // Check if user has role in metadata or database
      const role = meta.role || "user";
      setRole(role);
...
export const handleOAuthCallbackFromUrl = async () => {
  try {
    // This will automatically handle the URL fragment and establish the session
    const { data, error } = await supabase.auth.getSession();
