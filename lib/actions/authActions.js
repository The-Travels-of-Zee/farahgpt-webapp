...
const createUserProfile = async (user, role) => {
  try {
    const tableName = role === "instructor" ? "sellers" : "users";
    const userIdColumn = role === "instructor" ? "user_id" : "id";

    const googlePhoto =
      user.user_metadata?.photo_url ||
      user.user_metadata?.picture || // sometimes it's 'picture'
    ...
    const profileData = {
      ...
      ...(role === "user" && { subscription_tier: user.user_metadata?.subscription_tier || "free" }),
    };
    ...
    if (existingProfile) {
      // 🛑 Don't overwrite existing photo_url if not present from Google
      const updateData = {
...
export const handleOAuthCallbackFromUrl = async () => {
  try {
    ...
    login({
      ...
      email: user.email,
      initials: (meta.name || meta.full_name || user.email || "U").slice(0, 2).toUpperCase(),
      photo_url: meta.photo_url_url || meta.picture || null,
      provider: user.app_metadata?.provider || "google",
      isPremium: subscriptionTier === "premium",
    });
