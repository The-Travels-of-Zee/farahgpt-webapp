...
export const loginWithEmail = async (email, password) => {
  try {
    ...
    // Set premium status if available
  ...
  try {
    ...
    if (role === "instructor") {
      ...
      profileData = {
        ...baseData,
        id: user.id,
        photo_url: user.user_metadata?.photo_url || user.user_metadata?.picture || null,
...
export const handleOAuthCallbackFromUrl = async () => {
  try {
    // First, try to get session from URL hash/fragment
    ...
    // If no session, try to handle the auth callback from URL
    if (!sessionData.session) {
      ...
      if (!callbackData.user) {
        throw new Error("No user found in callback");
      }
    }
    ...
    // Get role from sessionStorage, URL params, or default to user
    const urlParams = new URLSearchParams(window.location.search);
    const role =
      urlParams.get("role") || (typeof window !== "undefined" ? sessionStorage.getItem("auth_role") : null) || "user";
    ...
    console.log("Processing OAuth callback for role:", role);
