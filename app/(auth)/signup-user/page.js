...
import { signUpUserWithEmail, signInWithGoogle, completeUserLogin } from "@/lib/actions/authActions";
    ...
    try {
      ...
      if (result.success) {
        if (result.isExistingAuth) {
          ...
          await completeUserLogin(result.user, "user");
        ...
        <motion.div variants={itemVariants} className="p-8 lg:p-12 flex flex-col justify-center">
          <div className="max-w-md mx-auto w-full">
            ...
            <motion.form variants={itemVariants} onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-12 pr-4 py-4 border-2 rounded-xl focus:outline-none transition-colors ${
                      errors.fullName ? "border-red-300 focus:border-red-500" : "border-gray-200 focus:border-secondary"
                    }`}
                  />
                </div>
                {errors.fullName && (
              ...
              <div>
                <div className="relative">
                  <Tag className="absolute left-4 top-1/2 transform -translate-y-1/2 text-secondary w-5 h-5" />
                  <input
                    type="text"
                    placeholder="Promo Code (Optional)"
                    value={formData.promoCode}
                    onChange={(e) => handleInputChange("promoCode", e.target.value)}
                    className="w-full pl-12 pr-4 py-4 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-secondary transition-colors"
                  />
                </div>
              </div>
              ...
              <motion.div variants={itemVariants} className="text-start mb-2">
                <p className="text-gray-600">Want to Become an Instructor?</p>
              </motion.div>
              <Link href="/signup-instructor">
                <motion.button
                  ...
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  className="w-full bg-gradient-to-r mb-8 from-primary to-(--primary-light) text-white py-4 rounded-xl font-semibold text-lg shadow-lg hover:shadow-xl transition-all duration-300"
                >
